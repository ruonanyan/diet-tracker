...
function HomeAIBox({ onLogged, profile, tdee, frequentFoods = [] }) {
  ...
  const [atMatch, setAtMatch] = useState(null); // { node, atIdx, query }
  const [chipReplacing, setChipReplacing] = useState(null); // { el, top, left }
      ...
      {aiResult && (
        <div style={{ background: "#eaf2eb", borderRadius: 6, padding: "0.85rem", marginTop: "0.75rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
            ...
            <button className="wk-save-btn" onClick={logAiWorkout} disabled={saving}
              style={{ opacity: saving ? 0.4 : 1 }}>{saving ? "Saving…" : "Log"}</button>
          </div>
        </div>
      )}
